...
import study.spring.AroundHubSpringBoot.service.impl.ProductServiceImpl;
...
public class ProductControllerTest {
	...
	// ProductServiceImpl에서 잡고 있는 Bean 객체에 대해 Mock 형태의 객체를 생성해 준다.
	@MockBean
	...
	void getProductTest() throws Exception {
		...
		mockMvc.perform(
			...
			// jsonPath : http request 값이 존재하는지 확인
			// jsonPath의 depath가 깊어지면 .을 추가하여 탐색할 수 있다. (ex : $.productId.productIdName)
	...
	void createProductTest() throws Exception {
		// Mock 객체에서 특정 메소드가 실행되는 경우 실제 Return을 줄 수 없기 떄문에 아래와 같이 가정 사항을 만들어준다.
		...
		// 아래 코드로 json 형태 변경 작업을 대체할 수 있다.
